...
```shell
ts-scan check -o result.json --api-key <TrustSource API key> scan.json
```
> [!NOTE]
>
...
The option ```--vulns-confidence <level>``` can be used to control the confidence level (```high```, ```medium```, ```low```) for matching components with affected products listed in security bulletins, such as product/vendor tuples in CVEs. The default value is ```medium```. Sometimes it can be useful to apply a lower confidence level, e.g. as a  for example use the following command to search with a ```medium```confidence level:  

```shell
ts-scan check --vulns-confidence medium -o result.json --api-key <TrustSource API key> scan.json
```
